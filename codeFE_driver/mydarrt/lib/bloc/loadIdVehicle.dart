...
import 'dart:async';
import '../model/fetchIdVehicle.dart';
...
Future<List<String>> load2() async {
  fu_trips = fetchIdvehicleData('http://localhost:8082/api/container');
...
Future<List<String>> load1() async {
  fu_trips = fetchIdvehicleData('http://localhost:8082/api/coach');
  trips = await fu_trips;
  for (var trip in trips) {
